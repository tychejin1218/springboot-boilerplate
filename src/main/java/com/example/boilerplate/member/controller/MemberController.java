...
public class MemberController implements MemberControllerDocs {
  ...
  public BaseResponse<MemberDto.Response> getMember(@PathVariable Long id) {
    return BaseResponse.ok(memberService.getMember(id));
  }

  /**
   * 회원 저장
   *
   * @param insertMemberRequest 저장할 회원 정보가 포함된 MemberDto.Request 객체
   * @return 저장된 회원의 정보
   */
   ...
   * 회원 수정
   *
   * @param updateMemberRequest 수정할 정보가 포함된 MemberDto.Request 객체
   * @return 수정된 회원의 정보
   */
  ...
  public BaseResponse<MemberDto.Response> updateMember(
    ...
    return BaseResponse.ok(memberService.updateMember(updateMemberRequest));
  }
}
